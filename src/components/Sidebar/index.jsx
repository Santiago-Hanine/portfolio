...
export const Sidebar = () => {
  return (
        <nav className='fixed left-0 top-0 w-28 h-screen border-r-4 bg-black-bg border-skill-card hidden lg:flex flex-col items-center justify-around gap-16 z-50 '>
            <div>
                <a href="/">
            ...
            <ul className='flex flex-col  w-full'>
                {SidebarItems().map(item => (
                    <li key={item.name} className={`${item.classname ? 'border-b-skill-card  border-b-2' : ''} border-t-skill-card border-t-2 w-full h-20 flex items-center justify-center`}>
